...
individual-level model, but rather forms is the basis of *population-derived
...
repository such as the Human Connectome Project (HCP) (CITE) or Alzheimer's
Disease Neuroimaging Initiative (ADNI) (CITE).
...
[@mejia2020bayesian] (also CITE Mejia 2022 and 2025). An important feature of
...
(Figure 1). Here, our primary focus is creating and sharing population-derived
priors for using data from the HCP, based on various templates. These priors can
be directly used to perform BBM in studies of healthy young adults. To perform
BBM to study individuals from other populations, or using other templates, we
...
and individuals from an external study, the midnight scan club (MSC) (CITE). The
...
#Nohelia, can you add Figure captions and dynamic references in the text?
```
...
spatial topography in ${s}_{qv}$, as described in [CITE Mejia 2020]:
...
higher computational cost (CITE Mejia 2022). Note that structured noise may
...
included in the model because can be removed beforehand as described in [CITE
Mejia 2020] or via other methods like ICA-FIX.

The third level of the mdoel incorporates the population-derived prior on the
...
See [CITE Mejia 2025] for details on the population-derived prior on the
...
ICA maps (Table 1). Specifically, we use the 17-network Yeo parcellation (CITE),
the MSC group parcellation (CITE and get the real name), and the HCP group ICA maps
...
duration after motion scrubbing [Nohelia, what FD threhold was used?] and
dropping the first 15 frames. Second, we exclude any related subjects. Finally,
we balace sex within age groups. See [Appendix B] for details.  After all
filters, our final sample contains data from X [Nohelia, how many?  Can be
approximate if it differs across LR/RL/combined] subjects, which is used to
estimate the priors. [Nohelia, we are missing the information about which runs
were actually used for the LR, RL and combined priors.  For LR I guess it must
be REST1_LR and REST2_LR, and same for RL, but what about combined?]
...
# ````````````````````
# @Nohelia, can you report the FD threshold used, both here and above in the 
# Methods section where the filters are mentioned?
# ````````````````````
